...
class NSSpeechSynthesizerTimer : NSObject, NSSpeechSynthesizerDelegate
{
	...
	func start() -> Bool
	{
		...
		let synthesizer = NSSpeechSynthesizer(voice: self.synthesizer.voice())
		{
            ...
            synthesizer.startSpeaking(self.text, to: NSURL(fileURLWithPath: tempFile) as URL)
    ...
    private func speechSynthesizer(sender: NSSpeechSynthesizer, didFinishSpeaking finishedSpeaking: Bool)
	{
